...
// Assignment 6
// wordCount.h
// wordCount.h is the header file for the wordCount class, it contains all function definitions for the class, as well as the class variables

#ifndef ast5_wordCount_h
#define ast5_wordCount_h
...
public:
    ...
    // successful read the document one word. Each word should be added to to the red-black tree which will create new nodes for unique words and update the count for duplicate
    // words.The function should remove any brackets, numbers, or punctuation from the words
    // (e.g., '(', '{', '[', ')', '}', ']', '!', '?', '.', ':', '$', '#', etc.). Additionally, numbers should be
    
    // filtered out. For example, 'Hello!' would be stored as 'hello'. All words should be converted to lower case. This will count 'Hello' and 'hello' as the same word.
    // Additionally, the word '1234', 'p@asw0rd', 're­did', and 'count12er' should be ignored (i.e. not stored in the tree). The function should update the total word count
    // (including duplicate words). See the sample output for examples. If the open and read are successful, the function should return true and false otherwise.
